...
class TestAzureTextAnalyticsClient:
    ...
    def mock_text_analytics_client(self):
        ...
        with patch("src.clients.azure_text_analytics.AzureTextAnalyticsClient._get_text_analytics_client") as mock_client:
            yield mock_client
    ...
    @pytest.fixture
    def text_analytics_client(self, mock_text_analytics_client: MagicMock) -> AzureTextAnalyticsClient:
    ...
    def test_successful_response(
        ...
        mock_text_analytics_client.return_value.analyze_sentiment.return_value = [mock_response]
    ...
    def test_document_error_response(
        ...
        mock_text_analytics_client.return_value.analyze_sentiment.return_value = [mock_error]

        with pytest.raises(AzureTextAnalyticsClientError, match="Text Analytics API error"):
    ...
    def test_http_response_error(
        ...
        mock_text_analytics_client.return_value.analyze_sentiment.side_effect = HttpResponseError("Service unavailable")

        with pytest.raises(AzureTextAnalyticsClientError, match="Text Analytics API error: Service unavailable"):
            text_analytics_client.analyze_sentiment("Some text")

        mock_logger.assert_called_once_with("Text Analytics API error: Service unavailable")
    ...
    def test_authentication_error(
        ...
        mock_text_analytics_client.return_value.analyze_sentiment.side_effect = ClientAuthenticationError("Invalid API Key")

        with pytest.raises(AzureTextAnalyticsClientError, match="Text Analytics API error: Invalid API Key"):
    ...
    def test_unexpected_exception(
        ...
        mock_text_analytics_client.return_value.analyze_sentiment.side_effect = Exception("Unexpected failure")

        with pytest.raises(AzureTextAnalyticsClientError, match="Text Analytics API error: Unexpected failure"):
            text_analytics_client.analyze_sentiment("Test prompt")

        mock_logger.assert_called_once_with("Text Analytics API error: Unexpected failure")
    ...
    def test_sentiment_processing_exception(
        ...
        with pytest.raises(AzureTextAnalyticsClientError, match="Sentiment analysis failed"):
